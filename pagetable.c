...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		if (coremap[frame].pte->frame & PG_DIRTY) {
			int off_set = swap_pageout(coremap[frame].pte->frame >> PAGE_SHIFT,
										coremap[frame].pte->swap_off);

			int off_set = swap_pageout(coremap[frame].pte->frame >> PAGE_SHIFT,
			                                coremap[frame].pte->swap_off);

            // assign pte swap_off as returned above.
			coremap[frame].pte->swap_off = off_set;

			// page is invalid or no longer on physical memory.
			...
			// We mark it as invalid because if it is clean that means it should
			// not be written on swap file but we have to indicate that later if
			// we call this page again then it should know where it is been stored.
            coremap[frame].pte->frame = coremap[frame].pte->frame & (~PG_VALID);
	...
	// because this is the new added page/frame, so its the most recent referenced one.
	coremap[frame].count_ref = 0;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	uintptr_t scnd_pgtbl = pgdir[idx].pde & PAGE_MASK ;

	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	p = (pgtbl_entry_t *) scnd_pgtbl + PGTBL_INDEX(vaddr);
	...
	if(p->frame & PG_VALID){
		...
		if(p->frame & PG_ONSWAP){
			...
			swap_pagein(allocated_frame, p->swap_off);
			// store this allocated_frame to p.
			p->frame = allocated_frame << PAGE_SHIFT;
			// make that frame set to swap on.
			p->frame = p->frame | PG_ONSWAP;
			// mark it clean as it is been retrieved from the swapfile.
			p->frame = p->frame & ~PG_DIRTY;

		} else {
			// this is the first time reference to this page so frame should be 
		    // allocated and initialized.
			int allocated_frame = allocate_frame(p);
			
			// init_frame(allocated_frame >> PAGE_SHIFT, vaddr);
			init_frame(allocated_frame, vaddr);
			// store this allocated frame to p.
			p->frame = allocated_frame << PAGE_SHIFT;
			// mark p as dirty because it is now available on physical memory.
			p->frame = p->frame | PG_DIRTY;
		}
	}
